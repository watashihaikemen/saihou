document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 状態保存・読込 ---
    function saveState() {
        localStorage.setItem('sewingToolState', JSON.stringify(state));
    }
...
function resetState() {
        ...
        for (const skillKey in SKILLS) {
            ...
            for (const target of targets) {
                ...
                target.indices.forEach(index => {
                    ...
                    if (skillKey === 'nerai') {
                         const critRate = state.baseCritRate + 0.20; // ねらいぬいの会心率上昇分（仮定値）
                         const normalVal = tempGrid[index] - finalReduction;
                         const critVal = 0; // ねらい会心は基本0狙い
                ...
                // 評価基準: 全マスの残り数値の絶対値の合計が最も小さいものを採用
                const newScore = tempGrid.reduce((sum, val) => sum + Math.abs(val), 0);

                if (newScore < bestAction.score) {
                    bestAction = {
                        score: newScore,
                        skillName: https://t.co/eN7m5x3tB3,
                        targetName: https://t.co/Fp7eIEdtib,
                        cost: skill.cost,
                    };
                }
            }
    ...
    function getTargetsForSkill(skillKey) {
        // (この関数は変更なし)
        const targets = [];
        if (['kagen', 'normal', 'double', 'triple', 'nerai'].includes(skillKey)) {
            for (let i = 0; i < 9; i++) {
                if (state.gridValues[i] > 0) targets.push({ name: CELL_NAMES[i], indices: [i] });
            }
    ...
    // --- 補助UI関数 ---
    function updateRegenerationNotice() {
        const turn = parseInt(document.getElementById('current-turn').value);
        ...
        if (turn > 0 && turn % 4 === 0) {
            ...
            if (targetIndex !== -1) {
                noticeEl.textContent = `再生ターン！ ${CELL_NAMES[targetIndex]} が12～16回復します。`;
            } else {
                 noticeEl.textContent = '再生ターンですが、回復対象のマスがありません。';
            }
            https://t.co/tzCfuDc1sd.display = 'block';
        } else {
            https://t.co/tzCfuDc1sd.display = 'none';
        }
    }
    
    function updateCritTrackerUI() {
